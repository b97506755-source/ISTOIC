        ...
        return (
            <div className="h-[100dvh] bg-[#050505] flex flex-col p-4 md:p-8 relative font-sans text-white overflow-hidden">
                ...
                <SidebarIStokContact 
                    ...
                    sessions={sessions} 
                    profile={myProfile}
                    onSelect={(s) => { setTargetPeerId(s.id); if('pin' in s && s.pin) setAccessPin(s.pin); setShowSidebar(false); }}
                    onDeleteSession={(id) => setSessions(prev => prev.filter(s => s.id !== id))} 
                    onCallContact={(c) => { setTargetPeerId(c.id); setShowSidebar(false); setTimeout(() => document.querySelector('input[placeholder="PIN (6)"]')?.focus(), 300); }} 
                    onLogout={onLogout} 
                    onRenameSession={()=>{}} 
                    currentPeerId={null}
                />
